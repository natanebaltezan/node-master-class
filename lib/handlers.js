                  ...
                  userChecks.forEach(checkId => {
                    _data.delete('checks', id, (err) => {
                      if (err) {
                        ...
                        checksDeleteds++;
                        if (checksDeleteds == checksToDelete) {
                          if (!deletionErrors) {
                            callback(200);
                          } else {
                            callback(500, { 'Errors': 'Errors encountered while attempting to delete all of the user check\'s' })
                          }
                        }
                      }
                    });
                ...
                _data.read('users', checkData.userPhone, (err, userData) => {
                  if (!err && userData) {
                    ...
                    if (checkPosition > -1) {
                      ...
                      callback(500, { 'Error': 'Couldn\'t find the check on the users object' });
                    }
                  } else {
                    callback(500, { 'Error': 'Couldn\'t find the user who created the check' });
                  }
                });
              } else {
